...
{  
    ...
    {       
        ...
        if (notAuthorized || currentUserService?.CurrentUser?.ValidUnixTime == null || hasAllowAnonymous)
        {
            await next();
            return;
        }
            ...
            {            
                ...
                else            
                {
                    context.Result = new JsonResult(
                        Result.Failed(
                            ServiceError.WithCode(
                                "TOKEN_EXPIRED",
                                currentTranslateService.TranslateSystemMessageByKey(
                                    KeySystemMessage.TokenExpired))))
                                    {
