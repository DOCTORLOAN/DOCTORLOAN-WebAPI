...
namespace DoctorLoan.Application.Common.Helpers;public class CommonHelper
{   
    ...
    {      
        return Enum.GetValues(typeof(T)).Cast<T>().Select(v => new SelectListItemModel
        { 
            Text = v.GetDescription(), Value = Convert.ToInt32(v).ToString() 
        }).ToList();    
    }   
