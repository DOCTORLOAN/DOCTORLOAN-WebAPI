...
public class AddMedicalRecordCommandHandler : ApplicationBaseService<AddMedicalRecordCommandHandler>, IRequestHandler<AddMedicalRecordCommand, Result<int>>
{
    private readonly IMapper _mapper;
    private readonly IEmailSenderService _emailSender;

    public AddMedicalRecordCommandHandler(ILogger<AddMedicalRecordCommandHandler> logger, IApplicationDbContext context,
    ...
    public async Task<Result<int>> Handle(AddMedicalRecordCommand request, CancellationToken cancellationToken)
    {
        ...
        var newMedicalReord = _mapper.Map<Domain.Entities.MedicalRecord.MedicalRecord>(request);
        newMedicalReord.Customer = customer;
        if (customerAddress is not null)
            newMedicalReord.CustomerAddresses = customerAddress;
        newMedicalReord.Status = Domain.Enums.Commons.StatusEnum.Publish;

        await _context.MedicalRecords.AddAsync(newMedicalReord, cancellationToken);
        ...
        try
        {
            if (!string.IsNullOrEmpty(customer.Email))
            {
                ...
                var content = "Thêm hồ sơ bệnh án thành công <br/>.";

                var message = new MessageEmail(to, "Tạo bệnh án thành công", content);
        ...
        {
            _logger.LogError($"Send email medical record error with customer: {customer.Id}, medicalRecord = {newMedicalReord.Id}");
        }

        return Result.Success(newMedicalReord.Id);
    }

    private async Task<CustomerAddress> HandleAddress(AddMedicalRecordCommand request, Domain.Entities.Customers.Customer customer, CancellationToken cancellationToken)
    {
